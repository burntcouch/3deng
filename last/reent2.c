...
typedef struct _navprog {
	...
	int cmd;		// 0 - nop
				// 1 - prog ctrl code
				// 4 - re-orient ship 
				// 5 - traverse ship orientation to vector
				// 9 - recalc orbit
				// 11 - main engine
				// 12 - attitude jets
	double p1;		// parm1
	double *xp;		// extra parms for above
	double *nvdOrbit;	// pointer to current orbit
...
void nvdInitNavProg(pnvdProg nprog, int icode);
...
int main(int argc, char *argv[]) {
	...
	double q_ship, qv_ship;  // calc from elements and convert to MKS
	...
	double g_Gany, g_Eur;  // calc from above for Reenter Calc
	...
	fn = 0.6378 * 0.6378;  // square of radius of Earth in EM_DU's
	fl = R_Gany + 0.01;  // 100 km orbit
	g_Gany = fn * M_Gany / (fl * fl);
	fl = R_Eur + 0.01;  // 100 km orbit
	g_Eur = fn * M_Eur / (fl * fl);
	vrot = sqrt(pow(a_Gany, 3.0)/M_Jup) * EM_TU * 3600;
	vrot = 2 * PI * R_Gany * EM_DU / vrot;
